...
async function processSign() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            status.innerHTML = `✅ <b>Підпис збережено як ${fileName}</b><br><br><b>Значення підпису (Hex):</b><br><textarea rows="3" readonly style="margin-top:5px; width:100%;">${hexSignature}</textarea>`;
            status.className = "status success-text";
        }
    } catch (e) {
...
async function processVerify() {
    ...
    try {
        ...
        if (data.error) {
            ...
            status.innerText = "✅ ПІДПИС ДІЙСНИЙ. Дані не змінено.";
            status.className = "status success-text";
        } else {
            status.innerText = "⛔ ПІДПИС НЕ ДІЙСНИЙ! Дані змінено або підроблено.";
            status.className = "status error-text";
        }
    } catch (e) {
